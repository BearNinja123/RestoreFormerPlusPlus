...
if __name__=='__main__':
    ...
    dataset = FFHQUnevenDegradationDataset(opt['data']['params']['train']['params'])

    for i in range(8):
        sample = dataset.getitem(i)
        name = sample['gt_path'].split('/')[-1][:-4]
        gt = tensor2img(sample['gt'], min_max=[-1, 1])
        # imwrite(gt, '/mnt/lustre/wangzhouxia/'+name+'_gt.png')
        lq = tensor2img(sample['lq'], min_max=[-1, 1])
        # mask = tensor2img(sample['mask'], min_max=[0, 1])
        
        # imwrite(lq, '/mnt/lustre/wangzhouxia/'+name+'_lq_nojitter.png')
        # imwrite(mask, name+'_mask.png')
        imwrite(gt, name+'_gt.png')
        imwrite(lq, name+'_lq_nojitter.png')
        # imwrite(gt[248:348, 197:297], name+'_n.png')

        # import pdb
        # pdb.set_trace()
        # components = get_roi_regions(sample['gt'].unsqueeze(0), sample['lq'].unsqueeze(0), 
        #                              sample['loc_left_eye'].unsqueeze(0), 
        #                              sample['loc_right_eye'].unsqueeze(0), 
        #                              sample['loc_mouth'].unsqueeze(0))
        # imwrite(tensor2img(components['left_eyes_gt'], min_max=[-1, 1]), name + '_left.png')
        # imwrite(tensor2img(components['right_eyes_gt'], min_max=[-1, 1]), name + '_right.png')
        # imwrite(tensor2img(components['mouths_gt'], min_max=[-1, 1]), name + '_m.png')
        # 
