...
if __name__ == '__main__':
    ...
    #x = torch.randn((2, C, 4, 4)).cuda().requires_grad_(True).contiguous(memory_format=torch.channels_last) * 100
    #gn1 = GN_NHWCRef(4, C).cuda()
    #gn2 = GN_NHWC(4, C).cuda()

    ##x = torch.randn((2, C, 4, 4)).cuda().requires_grad_(True) * 100
    ##gn1 = nn.GroupNorm(4, C).cuda()
    ##gn2 = GN_NCHW(4, C).cuda()
