...
void gn_nhwc_forward_kernel5(
  ...
  printf("TPB %d Bdx %d bdy %d f %d\n", TPB, blockDimX, blockDimY, f);
  compute_stats5_pt1<<<dim3(N, H, f), dim3(blockDimX, blockDimY)>>>(
      X_data, H, W, C, G, 
      welford_data
  );
  ...
  blockDimY = TPB / blockDimX;
  printf("TPB %d Bdx %d bdy %d f %d\n", TPB, blockDimX, blockDimY, f);
  compute_stats5_pt2<<<dim3(N, f), dim3(blockDimX, blockDimY)>>>(
          welford_data,
          H, G, eps,
          mean_data, rstd_data
    );
